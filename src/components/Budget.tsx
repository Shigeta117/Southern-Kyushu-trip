...
export function Budget() {
  ...
  return (
    <section id="budget" className="section-anchor">
      <SectionHeading icon="wallet" title="費用の目安" sub="1人あたり・4名割り・2026年夏想定" />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex items-baseline justify-between gap-3 p-4 bg-brand-50 border-b border-brand-100">
          ...
          <div className="text-sm font-bold text-brand-700 text-right shrink-0">
        ...
        <ul>
          {commonBudgetRows.map((r, i) => (
            <li key={i} className="flex items-baseline justify-between gap-3 p-4 border-b border-slate-100">
              <div className="min-w-0">
                <div className="text-sm font-medium text-slate-800">{r.item}</div>
                {r.note && <div className="text-xs text-slate-500 mt-0.5">{r.note}</div>}
              </div>
              <div className="text-sm font-semibold text-slate-700 text-right shrink-0">{r.amount}</div>
            </li>
          ))}
        </ul>
        ...
        <div className="flex items-center justify-between gap-3 p-4 bg-slate-900 text-white">
          ...
          <div className="font-black text-base sm:text-lg text-right">{origin.total}</div>
        </div>
      </div>

      <div className="flex gap-2 mt-3 text-xs text-amber-900 bg-amber-50 border border-amber-200 rounded-xl p-3 leading-relaxed">
