  ...
  return (
    <section id="emergency" className="section-anchor">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {emergencyNumbers.map((n) => (
          <a
            key={n.tel}
            href={`tel:${n.tel}`}
            className="bg-rose-600 text-white rounded-2xl p-3 text-center active:scale-95 transition-transform"
          >
            <div className="text-2xl font-black leading-none">{n.tel}</div>
      ...
      <div className="space-y-3 mb-4">
        {hazards.map((h) => (
          <div key={h.title} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex gap-3">
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-amber-100 text-amber-700 shrink-0">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-4">
        <div className="flex items-center gap-2 mb-3">
          <Icon name="hospital" className="w-5 h-5 text-rose-600 shrink-0" />
          <h3 className="font-bold text-slate-900">主要病院</h3>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-bold text-slate-900">公式リンク（運航・火山・天気）</h3>
        </div>
