...
export const hazards: Hazard[] = [
  {
    ...
    body: '台風18号ソウデル（非常に強い）は 8/25 15:00 に屋久島の南 605km、8/26 に 577km を通る予報で、九州には来ない。危ないのは台風そのものではなく先に届くうねりのほう。種子島・屋久島地方の波浪予報は 8/23「1m 後 1.5m」→ 8/24「1.5m 後 3m うねりを伴う」と上がる（8/22 17:00 発表）。⇒ 判断は降水確率ではなく波高で行う。運航の最終決定は出港1時間前。怪しければ前夜のうちに鹿児島空港→屋久島空港のJAC便（8:35→9:15 / 10:30→11:10 / 13:25→14:05 / 15:15→15:55 / 17:45→18:25）へ切り替える。渡れなかったときの損害は、①船代 ¥42,000 は欠航なら無料で戻る ②宿（たけすぎ ¥24,900）は天候理由でも当日100%で戻らない ③帰りの航空券（JAL セイバー乗継・3名 ¥101,010）は「出発前」に全区間を取り消せば税抜運賃額の約5%＝5,000円前後で済む。⚠️ ただし no show にすると「出発後」扱いで100%。渡れないと分かったら放置せず 8/26 18:40（出発20分前）までに必ず取り消すこと。飛行機ごと欠航するレベルなら会社都合の無手数料振替が使えるので、その時はD3・D4を鹿児島＋指宿に差し替える「プランB」へ。',
  },
  {
    icon: 'flame',
    title: '阿蘇はレベル3（入山規制）',
