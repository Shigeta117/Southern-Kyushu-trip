...
export const transportCards: TransportCard[] = [
  {
    id: 'rentacar',
    icon: 'car-front',
    title: '✅ レンタカー ― 延長成立（熊本空港で返却）',
    lead: '8/24 朝の交渉で全部通った。しかも乗り捨て料金が消えて安くなった。',
    rows: [
      ...
      { label: '✅ 変更後', value: '阿蘇くまもと空港で返却へ変更（乗り捨て先の変更＋延長）' },
      { label: '💰 料金', value: '37,840円 → 31,000円ちょっと。県外乗り捨てのオプション料金が消えて約6,800円の減' },
    ],
    notes: [
      '💡 効いたのは聞く順番だった。「延長したらいくら」ではなく「県外乗り捨て料金がいくら乗っているか」から入った。熊本空港へ戻せばそれが消えるので、延長代と相殺どころか逆転する。',
      '⚠️ 返却の日時はフライトに合わせて 8/25 の夕方（17:30 目安）。せきけんが取った内容を確認しておくこと。',
      '8/25 は桜島のあと熊本まで自走。九州道 鹿児島IC→益城熊本空港IC は 4,240円、ガソリン約1,600円。',
      '⚠️ 返却前に給油する。阿蘇くまもと空港周辺のスタンドを事前に見ておくこと。',
  ...
  },
  {
    id: 'no-car',
    icon: 'route',
    title: '【参考】公共交通で動く場合の足',
    lead: 'レンタカーを延長できたので今回は使わない。参考として残す。',
    rows: [
    ...
    notes: [
      ...
      '8/25 は 19:20 発のジェットスターに対して 17:30 には空港へ入りたい。',
  ...
  },
  {
    ...
    rows: [
      ...
      { label: '目的地', value: '8/27 に千葉の実家で用事。成田のほうが羽田より近い' },
    ],
    notes: [
  ...
  },
  {
    ...
    lead: '屋久島発の券なので、行かない以上は乗れない。8/25 の夜にはもう東京にいるので、放置が一番損をする。',
    rows: [
    ...
    notes: [
      ...
      '⭐️ 先に確認する価値があるのが「悪天候による特別対応」。JAL は台風接近時に対象空港を指定し、実際に欠航したかどうかに関わらず手数料無料で払戻・変更を認める。屋久島または鹿児島が対象空港に入っていれば手数料がゼロになる。',
      '❌ 「8/26 に欠航するのを待って無手数料払戻を狙う」は分が悪い。得られるのは最大5,000円で、忘れたら101,010円。しかも 8/26 はもう旅行が終わっている。',
    ],
    links: [
